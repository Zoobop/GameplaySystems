...
public class ItemSlotDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        position = EditorGUI.PrefixLabel(labelPosition, GUIUtility.GetControlID(FocusType.Passive), new GUIContent(itemName));
        ...
        var amountPropPos = new Rect(position.x + widthSize + labelOffset + 10, position.y, widthSize, position.height);
        var maxStackPos = new Rect(position.x + labelOffset + 20 + widthSize * 2, position.y, widthSize - 20, position.height);
